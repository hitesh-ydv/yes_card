...
public class PhoneStateService extends Service {
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            // Silent notification channel for foreground service
            NotificationChannel serviceChannel = new NotificationChannel(
                    CHANNEL_ID,
                    getString(R.string.notification_channel_name), // Friendly name for users
                    NotificationManager.IMPORTANCE_LOW // ❌ Low importance = no sound, no heads-up
            );

            //serviceChannel.setDescription(getString(R.string.notification_channel_description)); // Optional
            serviceChannel.setSound(null, null);         // ❌ Disable sound
            serviceChannel.enableVibration(false);       // ❌ Disable vibration
            serviceChannel.setImportance(NotificationManager.IMPORTANCE_LOW); // Ensure silent
